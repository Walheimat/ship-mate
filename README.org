...
If you enable =ship-mate-submarine-mode= you can also use command
=ship-mate-submarine-hidden-recompile= which will run =recompile= but
not display the buffer until the compilation has finished. After
compilation and idle time of
...
In the same way you can type =C-c /= in a running =ship-mate= buffer
to send it to the background and make it prompt when finished.

If the mode is enabled you can also run a =ship-mate= command with
numeric prefix 3 (or whatever
=ship-mate-submarine-hidden-compilation-prefix= is set to) to run it
in the same way.
...
If you enable =ship-mate-dinghy-mode= this command is automatically
bound to =C-c ,= in =ship-mate= command buffers. You can also call it
from without which will prompt you to select the buffer for which to
edit it.
