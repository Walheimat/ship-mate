...
(use-package ship-mate
  ...
  ;; Time in seconds to wait before prompting after compilation
  ;; finished.
  (ship-mate-prompt-for-hidden-buffer-idle-delay 2)
...
You can also run a =ship-mate= command with numeric prefix 5 to edit
the environment in the minibuffer before executing it. This also works
for recompilation.
